var dY=dY||{};dY.Arr=function(meta,ticks){this.meta=meta;this.ticks=ticks;}
...
ticks=[];results.data.forEach(function(row,n){hourOfYear=dY.datetime.dateToHourOfYear(dY.datetime.dateStringToDate(row["Date/Time"]));data={};for(var zon in meta){data[zon]={};for(var key in meta[zon]){value=row[dY.parser.zoneKeyToString(zon,key)];data[zon][key]=value;alls[[zon,key]].push(value);}}
ticks.push(new dY.Tick(hourOfYear,data));});for(var zon in meta){for(var key in meta[zon]){allsorted=alls[[zon,key]].sort(function(a,b){return a-b});len=allsorted.length;meta[zon][key].min=allsorted[0];meta[zon][key].q1=allsorted[Math.floor(len*.25)-1];meta[zon][key].q2=allsorted[Math.floor(len*.50)-1];meta[zon][key].q3=allsorted[Math.floor(len*.75)-1];meta[zon][key].max=allsorted[len-1];meta[zon][key].domain=[meta[zon][key].min,meta[zon][key].max];meta[zon][key].median=meta[zon][key].q2;sum=0;for(var i=0;i<allsorted.length;i++){sum+=allsorted[i];}
...
meta={EPW:{}};dY.parser.EPWKeyDefs.forEach(function(keyDef){meta["EPW"][keyDef.key]={};});dY.report("dy: Parser found "+results.data.length+" rows. We expect this to represent a full year of 8760 hours.")
alls=[];for(var key in meta["EPW"]){alls[["EPW",key]]=[];}
ticks=[];results.data.forEach(function(row,n){datestring=dY.util.pad(row[1])+"/"+dY.util.pad(row[2])+"  "+dY.util.pad(row[3])+":00"
hourOfYear=dY.datetime.dateToHourOfYear(dY.datetime.dateStringToDate(datestring));data={};data["EPW"]={};dY.parser.EPWKeyDefs.forEach(function(keyDef){value=row[keyDef.col];data["EPW"][keyDef.key]=value;alls[["EPW",keyDef.key]].push(value);});ticks.push(new dY.Tick(hourOfYear,data));});for(var key in meta["EPW"]){allsorted=alls[["EPW",key]].sort(function(a,b){return a-b});len=allsorted.length;meta["EPW"][key].min=allsorted[0];meta["EPW"][key].q1=allsorted[Math.floor(len*.25)-1];meta["EPW"][key].q2=allsorted[Math.floor(len*.50)-1];meta["EPW"][key].q3=allsorted[Math.floor(len*.75)-1];meta["EPW"][key].max=allsorted[len-1];meta["EPW"][key].domain=[meta["EPW"][key].min,meta["EPW"][key].max];meta["EPW"][key].median=meta["EPW"][key].q2;sum=0;for(var i=0;i<allsorted.length;i++){sum+=allsorted[i];}
