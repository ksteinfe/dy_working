...
var doIt=function(filedata){var headCount=2;var combinedContentRows=new Array(8760);var combinedHeaderRows="Date/Time";var init=true;for(var filename in filedata){var fileContent=filedata[filename];var splitContent=fileContent.split("\n");headRows=splitContent.slice(0,2);contentRows=splitContent.slice(2);var firstSplitHeadRow=headRows[0].split(",");var secondSplitHeadRow=headRows[1].split(",");if(firstSplitHeadRow[firstSplitHeadRow.length-1].trim()=="")firstSplitHeadRow.pop();if(secondSplitHeadRow[secondSplitHeadRow.length-1].trim()=="")secondSplitHeadRow.pop();var fname=filename.slice(0,-4);for(var col=1;col<firstSplitHeadRow.length;col++){var str=fname+":"+firstSplitHeadRow[col]+"["+secondSplitHeadRow[col]+"]";combinedHeaderRows=combinedHeaderRows+","+str;}
if(init){for(var row=0;row<(contentRows.length-1);row++){var month=doubleDigit(contentRows[row].slice(0,contentRows[row].indexOf("/")));var day=doubleDigit(contentRows[row].slice(contentRows[row].indexOf("/")+1,contentRows[row].lastIndexOf("/")));if(contentRows[row].indexOf(" ")==-1){var loc=contentRows[row].length;}else{var loc=contentRows[row].indexOf(" ");}
if(contentRows[row].indexOf(" ")==-1){var hour=24;}else{var hour=contentRows[row].slice(contentRows[row].indexOf(" "),contentRows[row].indexOf(" ")+2).trim();if((contentRows[row].slice(contentRows[row].lastIndexOf(" "),contentRows[row].lastIndexOf(" ")+2)=="PM")&&hour!='12'){hour=hour+12;}}
