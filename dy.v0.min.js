...
var dY=dY||{};dY.Tick=function(hourOfYear,data){this.hourOfYear=hourOfYear;this.data=data;}
dY.Tick.prototype.hourOfDay=function(){return this.hourOfYear%24;};dY.Tick.prototype.dayOfYear=function(){return Math.floor(this.hourOfYear/24);};dY.Tick.prototype.valueOf=function(zonekey){return this.data[zonekey[0]][zonekey[1]];};var dY=dY||{};dY.parser={};function onDataLoaded(data,fields){dY.report("dhr: THIS IS A PLACEHOLDER FUNCTION - REPLACE WITH YOUR OWN");}
...
schema={};if(results.schema.fields.length>0){dY.report("dy: Parser found "+results.schema.fields.length+" columns (not including Date/Time)")
zoneStrings=new Set();results.schema.fields.forEach(function(field,n){if(!dY.parser.stringToZoneKey(field))return;zoneStrings.add(dY.parser.stringToZoneKey(field)[0]);});zoneStrings=Array.from(zoneStrings);zoneStrings.forEach(function(zoneStr,n){schema[zoneStr]=[];results.schema.fields.forEach(function(field,n){key=dY.parser.stringToZoneKey(field);if(key&&key[0]==zoneStr)schema[zoneStr][key[1]]=[];});});}
dY.report("dy: Parser found "+results.data.length+" rows. Parser doesn't care about the number of rows nor their order.")
alls=[];for(var zon in schema){for(var key in schema[zon]){alls[[zon,key]]=[];}}
ticks=[];results.data.forEach(function(row,n){hourOfYear=dY.datetime.dateToHourOfYear(dY.datetime.dateStringToDate(row["Date/Time"]));data={};for(var zon in schema){data[zon]={};for(var key in schema[zon]){value=row[dY.parser.zoneKeyToString(zon,key)];data[zon][key]=value;alls[[zon,key]].push(value);}}
ticks.push(new dY.Tick(hourOfYear,data));});for(var zon in schema){for(var key in schema[zon]){allsorted=alls[[zon,key]].sort(function(a,b){return a-b});len=allsorted.length;schema[zon][key].min=allsorted[0];schema[zon][key].q1=allsorted[Math.floor(len*.25)-1];schema[zon][key].q2=allsorted[Math.floor(len*.50)-1];schema[zon][key].q3=allsorted[Math.floor(len*.75)-1];schema[zon][key].max=allsorted[len-1];schema[zon][key].domain=[schema[zon][key].min,schema[zon][key].max];schema[zon][key].median=schema[zon][key].q2;sum=0;for(var i=0;i<allsorted.length;i++){sum+=allsorted[i];}
schema[zon][key].average=sum/len;}}
arr=new dY.Arr(schema,ticks)
if(typeof(callback)==='undefined'){return arr;}else{callback(arr);}}
...
alls=[];for(var key in schema["EPW"]){alls[["EPW",key]]=[];}
ticks=[];results.data.forEach(function(row,n){datestring=dY.util.pad(row[1])+"/"+dY.util.pad(row[2])+"  "+dY.util.pad(row[3])+":00"
hourOfYear=dY.datetime.dateToHourOfYear(dY.datetime.dateStringToDate(datestring));data={};data["EPW"]={};dY.parser.EPWKeyDefs.forEach(function(keyDef){value=row[keyDef.col];data["EPW"][keyDef.key]=value;alls[["EPW",keyDef.key]].push(value);});ticks.push(new dY.Tick(hourOfYear,data));});for(var key in schema["EPW"]){allsorted=alls[["EPW",key]].sort(function(a,b){return a-b});len=allsorted.length;schema["EPW"][key].min=allsorted[0];schema["EPW"][key].q1=allsorted[Math.floor(len*.25)-1];schema["EPW"][key].q2=allsorted[Math.floor(len*.50)-1];schema["EPW"][key].q3=allsorted[Math.floor(len*.75)-1];schema["EPW"][key].max=allsorted[len-1];schema["EPW"][key].domain=[schema["EPW"][key].min,schema["EPW"][key].max];schema["EPW"][key].median=schema["EPW"][key].q2;sum=0;for(var i=0;i<allsorted.length;i++){sum+=allsorted[i];}
