...
dY.Year.prototype.metaOf=function(zonekey){if(zonekey.constructor===Array)return this.schema[zonekey[0]][zonekey[1]];return this.schema[Object.keys(this.schema)[0]][zonekey];};dY.Year.prototype.valuesOf=function(zonekey){return this.ticks.map(function(d){return d.valueOf(zonekey);});};dY.Year.prototype.dailySummary=function(dayCount=1){var slcs=[];var t=0;while(t<this.ticks.length){stick=new dY.STick(dY.util.summarizeTicks(this.schema,ticks.slice(t,t+24*dayCount)));stick.setTickDomain([t,t+24*dayCount-1])
if(dayCount==1)stick.dayOfYear=t/24;slcs.push(stick);t+=24*dayCount;}
return slcs;};var dY=dY||{};dY.dt={};dY.dt.year=1970;dY.dt.dateStringToDate=function(str){splt=str.split("  ");date=splt[0].trim().split("/");month=parseInt(date[0])-1
day=parseInt(date[1])
...
dY.dt.monthTable=[{idx:0,fullname:"January",shortname:"Jan",msDomain:[Date.UTC(dY.dt.year,0,1,0,0,0,0),Date.UTC(dY.dt.year,1,1,0,0,0,0)]},{idx:1,fullname:"February",shortname:"Feb",msDomain:[Date.UTC(dY.dt.year,1,1,0,0,0,0),Date.UTC(dY.dt.year,2,1,0,0,0,0)]},{idx:2,fullname:"March",shortname:"Mar",msDomain:[Date.UTC(dY.dt.year,2,1,0,0,0,0),Date.UTC(dY.dt.year,3,1,0,0,0,0)]},{idx:3,fullname:"April",shortname:"Apr",msDomain:[Date.UTC(dY.dt.year,3,1,0,0,0,0),Date.UTC(dY.dt.year,4,1,0,0,0,0)]},{idx:4,fullname:"May",shortname:"May",msDomain:[Date.UTC(dY.dt.year,4,1,0,0,0,0),Date.UTC(dY.dt.year,5,1,0,0,0,0)]},{idx:5,fullname:"June",shortname:"Jun",msDomain:[Date.UTC(dY.dt.year,5,1,0,0,0,0),Date.UTC(dY.dt.year,6,1,0,0,0,0)]},{idx:6,fullname:"July",shortname:"Jul",msDomain:[Date.UTC(dY.dt.year,6,1,0,0,0,0),Date.UTC(dY.dt.year,7,1,0,0,0,0)]},{idx:7,fullname:"August",shortname:"Aug",msDomain:[Date.UTC(dY.dt.year,7,1,0,0,0,0),Date.UTC(dY.dt.year,8,1,0,0,0,0)]},{idx:8,fullname:"September",shortname:"Sep",msDomain:[Date.UTC(dY.dt.year,8,1,0,0,0,0),Date.UTC(dY.dt.year,9,1,0,0,0,0)]},{idx:9,fullname:"October",shortname:"Oct",msDomain:[Date.UTC(dY.dt.year,9,1,0,0,0,0),Date.UTC(dY.dt.year,10,1,0,0,0,0)]},{idx:10,fullname:"November",shortname:"Nov",msDomain:[Date.UTC(dY.dt.year,10,1,0,0,0,0),Date.UTC(dY.dt.year,11,1,0,0,0,0)]},{idx:11,fullname:"December",shortname:"Dec",msDomain:[Date.UTC(dY.dt.year,11,1,0,0,0,0),Date.UTC(dY.dt.year+1,0,1,0,0,0,0)]}]
dY.timeSpan=function(DateTimeStart,DateTimeEnd){if(Object.prototype.toString.call(DateTimeStart)=="[object Date]")DateTimeStart=+DateTimeStart
if(Object.prototype.toString.call(DateTimeEnd)=="[object Date]")DateTimeEnd=+DateTimeEnd
var coeff=1000*60;var a=Math.ceil(+DateTimeStart/coeff)*coeff;var b=Math.floor(+DateTimeEnd/coeff)*coeff;this._=dY.dt.niceFormat(new Date(a))+" -> "+dY.dt.niceFormat(new Date(b))
this.a=a;this.b=b;this.hourOfYearStart=function(){return Math.round(a/1000/60/60);};this.hourOfYearMid=function(){return((b-a)*0.5+a)/1000/60/60;};this.hourOfYearEnd=function(){return Math.round(b/1000/60/60);};this.hourOfYear=function(){return Math.floor(this.hourOfYearMid())};this.hourOfYearDomain=function(){return[this.hourOfYearStart(),this.hourOfYearEnd()-1];};this.hoursOfYear=function(){return Array.from(new Array(this.durationHrs()),(x,i)=>i+this.hourOfYearStart());};this.dayOfYear=function(){return Math.floor(this.hourOfYear()/24);};this.hourOfDay=function(){return this.hourOfYear()%24;}
this.duration=function(){return b-a;};this.durationSec=function(){return Math.round((b-a)/1000);};this.durationMin=function(){return Math.round((b-a)/1000/60);};this.durationHrs=function(){return Math.round((b-a)/1000/60/60);};this.isHour=function(){return this.durationHrs()==1;};}
dY.timeSpan.prototype.report=function(){console.log(this._);console.log("\t hoy domain \t\t"+this.hourOfYearDomain());console.log("\t mid hr \t\t"+this.hourOfYearMid());console.log("\t dur\t\t"+this.duration());console.log("\t durHrs\t\t"+this.durationHrs());console.log("\t hour of year\t\t"+this.hourOfYear());console.log("\t day of year\t\t"+this.dayOfYear());console.log("\t hour of day\t\t"+this.hourOfDay());};dY.timeSpan.hourOfYear=function(hr){return new dY.timeSpan(hr*(1000*60*60),(hr+1)*(1000*60*60))}
...
dY.timeSpan.monthOfYear=function(mth){return new dY.timeSpan(dY.dt.monthTable[mth].msDomain[0],dY.dt.monthTable[mth].msDomain[1]);};dY.timeSpan.monthsOfYear=function(a,b){return new dY.timeSpan(dY.dt.monthTable[a].msDomain[0],dY.dt.monthTable[b].msDomain[1]);};dY.timeSpan.fullYear=new dY.timeSpan(0,31536000000);dY.timeSpan.janurary=dY.timeSpan.monthOfYear(0);dY.timeSpan.february=dY.timeSpan.monthOfYear(1);dY.timeSpan.march=dY.timeSpan.monthOfYear(2);dY.timeSpan.april=dY.timeSpan.monthOfYear(3);dY.timeSpan.may=dY.timeSpan.monthOfYear(4);dY.timeSpan.june=dY.timeSpan.monthOfYear(5);dY.timeSpan.july=dY.timeSpan.monthOfYear(6);dY.timeSpan.august=dY.timeSpan.monthOfYear(7);dY.timeSpan.september=dY.timeSpan.monthOfYear(8);dY.timeSpan.october=dY.timeSpan.monthOfYear(9);dY.timeSpan.november=dY.timeSpan.monthOfYear(10);dY.timeSpan.december=dY.timeSpan.monthOfYear(11);var dY=dY||{};dY.graph={};dY.graph.addBoard=function(selector,boardDims){dY.report("dy: adding an SVG to the document inside "+selector);if(!boardDims.hasOwnProperty("margin"))boardDims.margin=10;if(typeof(boardDims.margin)=="number")boardDims.margin={top:boardDims.margin,right:boardDims.margin,bottom:boardDims.margin,left:boardDims.margin};if(!boardDims.hasOwnProperty("width")||!boardDims.hasOwnProperty("height")){if(!boardDims.hasOwnProperty("inWidth")||!boardDims.hasOwnProperty("inHeight")){console.log("dY.graph.addBoard requires a defined width and height or a defined inWidth and inHeight");return false;}
...
var dY=dY||{};dY.solarGeom={};dY.solarGeom.dailyAtGivenHour=function(loc,hourOfDay){var days=[...Array(365).keys()];var data=days.map(function(d){return dY.solarGeom.solarGeomAtHour(loc,d,hourOfDay);});return{location:loc,hourOfDay:hourOfDay,data:data}}
...
dY.solarGeom.geomSurroundingHour=function(loc,hourOfYear,resolution){return{location:loc,hourOfYear:false,data:[solarGeom,solarGeom,solarGeom,solarGeom]}}
