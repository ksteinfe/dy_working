...
ticks=[];results.data.forEach(function(row,n){hourOfYear=dY.datetime.dateToHourOfYear(dY.datetime.dateStringToDate(row["Date/Time"]));data={};for(var zon in schema){data[zon]={};for(var key in schema[zon]){value=row[dY.parser.zoneKeyToString(zon,key)];data[zon][key]=value;}}
ticks.push(new dY.Tick(hourOfYear,data));});schema=dY.util.summarizeTicks(schema,ticks);yr=new dY.Year(schema,ticks)
...
hourOfYear=dY.datetime.dateToHourOfYear(dY.datetime.dateStringToDate(datestring));data={};data["EPW"]={};dY.parser.EPWKeyDefs.forEach(function(keyDef){value=row[keyDef.col];data["EPW"][keyDef.key]=value;});ticks.push(new dY.Tick(hourOfYear,data));});schema=dY.util.summarizeTicks(schema,ticks);yr=new dY.Year(schema,ticks)
...
var hrDomain=[dY.datetime.dateToHourOfYear(Date.parse(headTypExtrmPeriods[p+2]+"/"+dY.datetime.year+" 00:30:00 UTC")),dY.datetime.dateToHourOfYear(Date.parse(headTypExtrmPeriods[p+3]+"/"+dY.datetime.year+" 23:30:00 UTC"))]
