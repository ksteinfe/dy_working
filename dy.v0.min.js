...
hrs=[];results.data.forEach(function(row,n){datestring=dY.util.pad(row[1])+"/"+dY.util.pad(row[2])+"  "+dY.util.pad(row[3])+":00"
hourOfYear=dY.datetime.dateToHourOfYear(dY.datetime.dateStringToDate(datestring));data={};data["EPW"]={};dY.parser.EPWKeyDefs.forEach(function(keyDef){value=row[keyDef.col];data["EPW"][keyDef.key]=value;alls[["EPW",keyDef.key]].push(value);});hrs.push(new dY.Hr(hourOfYear,data));});for(var key in meta["EPW"]){allsorted=alls[["EPW",key]].sort(function(a,b){return a-b});len=allsorted.length;meta["EPW"][key].min=allsorted[0];meta["EPW"][key].q1=allsorted[Math.floor(len*.25)-1];meta["EPW"][key].q2=allsorted[Math.floor(len*.50)-1];meta["EPW"][key].q3=allsorted[Math.floor(len*.75)-1];meta["EPW"][key].max=allsorted[len-1];meta["EPW"][key].domain=[meta["EPW"][key].min,meta["EPW"][key].max];meta["EPW"][key].median=meta["EPW"][key].q2;sum=0;for(var i=0;i<allsorted.length;i++){sum+=allsorted[i];}
